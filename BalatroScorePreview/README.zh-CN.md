...
- 选中的牌处于逻辑背面状态时，显示 `Reference: ???`、`参考值：？？？` 或 `參考值：？？？`，避免泄露被选中隐藏牌的信息。
...
- 修复预览试算在吸血鬼等效果移除增强牌后，没有完整恢复牌中心状态的问题。
- 防止预览计算把增强牌留下“普通牌中心 + 增强牌能力”的脏状态，从而导致牌说明显示 `ERROR`。
- 增加一个窄范围的旧存档修正：如果金卡、幸运牌等原版增强牌的能力数据还在，但牌中心已经被污染为普通牌，会自动恢复对应增强牌中心。
- 公开版本提升到 1.2.2。

### 1.2.1

- 修复内部渲染/动画字段被误判为隐藏状态，导致正面牌也显示未知值的问题。
- 将隐藏牌检测收紧为只判断逻辑背面状态。
- 公开版本提升到 1.2.1。

### 1.2.0

- 调整隐藏牌处理规则：只有被选中的背面牌或正在翻面的牌才会显示未知值。
- 公开版本提升到 1.2.0。
