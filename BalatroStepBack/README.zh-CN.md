...
Balatro Step Back / 对局回退 会在当前盲注内添加出牌/弃牌检查点。

- 英文 UI：`Back` 回到最近一次出牌或弃牌前，`History` 打开检查点列表。
- 简体中文 UI：`回退` 回到最近一次出牌或弃牌前，`记录` 打开检查点列表。
- 繁体中文 UI：`回退` 回到最近一次出牌或弃牌前，`記錄` 打开检查点列表。
- 历史列表会显示为“回到第 1 次出牌前”、“回到第 1 次使用消耗牌前”等更直观的格式。
- 历史记录仅限当前盲注。选择更早的检查点后，之后的检查点历史会被丢弃。
- 在盲注内使用持有的消耗牌前也会创建检查点，因此回退可以把塔罗牌、星球牌、幻灵牌等恢复回消耗牌区。
...
- 在盲注内使用持有的消耗牌前新增检查点。
- 修复塔罗牌等已使用的持有消耗牌在回退后没有回到消耗牌区的问题。
- 公开版本提升到 1.1.0。
