...
Upload file: `BalatroShopUndo-1.0.1.zip`

### Short Description

```text
Balatro Shop Undo / 商店回退 adds a small localized shop undo button for accidental purchases, buy-and-use actions, sales, and voucher redemptions. Rerolls and booster openings are excluded to avoid free previews; after opening a booster, safe earlier buy/sell checkpoints can still be undone while the opened pack and pack results stay unchanged.

Balatro Shop Undo / 商店回退 会在商店界面添加一个本地化回退按钮，用于撤回误购买、购买并使用、出售和兑换优惠券；不会回退重掷商店和补充包开包结果，但开包后仍会尽量保留之前安全的普通购买/出售回退点。
```
...
- Keeps safe earlier buy/sell checkpoints after a booster pack is opened, while booster purchases and pack results stay unchanged.
...
1. Install Lovely Injector.
2. Install Steamodded / SMODS.
3. Download BalatroShopUndo-1.0.1.zip.
4. Extract the zip.
...
- 在商店里出售卡牌前创建检查点。
- 兑换优惠券前创建检查点。
- 打开补充包后会保留安全的普通购买/出售回退点，同时补充包购买和开包结果保持不变。
...
1. 安装 Lovely Injector。
2. 安装 Steamodded / SMODS。
3. 下载 BalatroShopUndo-1.0.1.zip。
4. 解压压缩包。
...
Suggested category: User Interface or Utilities

Suggested tags: Quality of Life, User Interface, Run History

Upload file: `BalatroRunArchive-0.1.2.zip`

### Short Description

```text
Balatro Run History / 历史战绩 adds an Options-menu run history for reviewing previous runs, including deck, stake, seed, result, final state, and expandable small card previews.

Balatro Run History / 历史战绩 会在选项菜单添加历史战绩，用来回看之前每一局的卡组、难度、Seed、结果和最终状态，并可展开查看缩小牌面。
```
...
Balatro Run History / 历史战绩 adds a simple long-term run history to the Options menu. It is designed for players who want to remember what happened in previous runs without taking screenshots or writing notes by hand.

This first version focuses on stable, practical records rather than a huge stat tracker.

Main features

- Adds a Run History button to the Options menu.
- Creates a run record while you play.
- Records deck, stake, seed, seeded-run flag, challenge flag, result, ante, round, blind, money, and best single-hand score.
- Records final Jokers, redeemed vouchers, and the final playing-card deck snapshot.
- Final Jokers, deck cards, and vouchers can be expanded into small card previews.
- Shows basic history stats: wins, losses, current win streak, and best win streak.
- The details screen has Overview and Final State tabs.
- Supports English, Simplified Chinese, and Traditional Chinese UI text.
...
1. Install Lovely Injector.
2. Install Steamodded / SMODS.
3. Download BalatroRunArchive-0.1.2.zip.
4. Extract the zip.
5. Move the `BalatroRunArchive` folder into:
...
- This mod does not edit Balatro game files.
- Run history data is stored in Balatro's current profile/settings data and saved through Balatro's normal settings-save path.
- The history keeps up to 500 recent runs so profile data does not grow without limit.
- Advanced filtering and deeper statistics are planned for later versions.
- Runs started before installing this mod may only get partial records if continued later.
- Conflicts are possible with mods that replace the same start-run, shop, use-card, win, game-over, or Options-menu UI functions.
- If you hit a crash, test again with only Lovely, Steamodded, and this mod enabled, then include your Lovely crash log when reporting.
...
Code by ZhiSunian. No Balatro, Lovely, Steamodded, image, audio, save, or other third-party assets are included.

---

中文说明

Balatro Run History / 历史战绩 会在选项菜单里添加一个长期历史战绩入口。它适合想回看上一局、前几局到底发生了什么，但又不想手动截图或记笔记的玩家。

这个首版先做稳定、实用的记录功能，不急着做成非常复杂的统计系统。

主要功能

- 在选项菜单添加 Run History / 历史战绩入口。
- 游玩时自动生成对局记录。
- 记录卡组、难度、Seed、是否指定 Seed、是否挑战、胜负结果、底注、回合、盲注、金钱和最高单手得分。
- 记录结束时的小丑牌、已兑换优惠券和最终牌组快照。
- 最终小丑牌、牌组和优惠券可以展开成缩小牌面查看。
- 显示基础统计：胜利、失败、当前连胜、历史最高连胜。
- 详情页分为“概览 / 最终状态”两个页面。
- 支持英文、简体中文、繁体中文 UI 文本。
...
1. 安装 Lovely Injector。
2. 安装 Steamodded / SMODS。
3. 下载 BalatroRunArchive-0.1.2.zip。
4. 解压压缩包。
5. 将 BalatroRunArchive 文件夹放入：
...
- 本模组不会修改 Balatro 游戏文件。
- 历史战绩会写入 Balatro 当前配置档案，并通过 Balatro 原本的设置保存流程保存。
- 历史战绩最多保留最近 500 局，避免配置档案无限变大。
- 高级筛选和更深入的统计会留到后续版本。
- 安装本模组之前已经开始的旧对局，如果之后继续游玩，可能只能生成部分记录。
- 如果其他模组替换同一批开始对局、商店、用牌、胜利、失败或选项菜单 UI 函数，可能出现冲突。
- 如果发生崩溃，请先只启用 Lovely、Steamodded 和本模组复测，并在反馈时附上 Lovely 崩溃日志。
...
代码作者：ZhiSunian。本模组不包含 Balatro、Lovely、Steamodded、图片、音频、存档或其他第三方资产。
```
...
Upload file: `BalatroScorePreview-1.3.0.zip`

### Short Description

```text
Balatro Score Preview / 分数预览 shows a localized pre-play reference score for the selected hand, with a green target-reached highlight when the current score plus the preview is enough to clear the blind. Selected face-down or flipping cards show an unknown value. Probability effects are treated as not triggering, and disruptive vanilla Boss Blind pre-play effects are isolated from the live hand. Supports English, Simplified Chinese, and Traditional Chinese UI.

Balatro Score Preview / 分数预览 会在出牌前显示本地化的所选手牌参考分数，当当前分数加预览分数已经足够通过盲注时会用绿色高亮提示达标；选中背面牌或正在翻面的牌时显示未知值，概率效果在预览中按“不触发”处理，原版 Boss Blind 的随机弃牌/翻面等出牌前副作用不会真实作用到手牌，并支持英文、简体中文和繁体中文 UI。
```
...
- Highlights the reference row in green and adds Enough, 达标, or 達標 when the current round score plus the previewed score is enough to clear the blind.
...
1. Install Lovely Injector.
2. Install Steamodded / SMODS.
3. Download BalatroScorePreview-1.3.0.zip.
4. Extract the zip.
...
- 当当前回合分数加上预览分数已经足够通过盲注时，参考值条会绿色高亮，并显示 Enough、“达标”或“達標”。
...
1. 安装 Lovely Injector。
2. 安装 Steamodded / SMODS。
3. 下载 BalatroScorePreview-1.3.0.zip。
4. 解压压缩包。
