...
```text
Balatro Score Preview / 分数预览 shows a localized pre-play reference score for the selected hand. Probability effects are treated as not triggering in preview. Supports English, Simplified Chinese, and Traditional Chinese UI.

Balatro Score Preview / 分数预览 会在出牌前显示本地化的所选手牌参考分数，概率效果在预览中按“不触发”处理，并支持英文、简体中文和繁体中文 UI。
```
